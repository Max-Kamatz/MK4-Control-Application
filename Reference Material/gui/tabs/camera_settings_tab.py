...
class CameraSettingsTab(QWidget):
    ...
    def init_ui(self):
        """Initialize the Camera Settings tab UI with scrollable content."""
    ...
    def create_camera_profiles_group(self) -> QGroupBox:
        """Group 14: Camera Profiles."""
        group = QGroupBox("Camera Profiles")
        layout = QGridLayout()
        layout.setSpacing(8)

        # Profile Number
        layout.addWidget(QLabel("Profile Number:"), 0, 0)
        self.camera_profile_input = QLineEdit()
        self.camera_profile_input.setPlaceholderText("0-N")
        layout.addWidget(self.camera_profile_input, 0, 1)
        self.camera_profile_button = QPushButton("Set")
        layout.addWidget(self.camera_profile_button, 0, 2)

        # Query Settings
        self.query_camera_settings_button = QPushButton("Query Camera Settings")
        layout.addWidget(self.query_camera_settings_button, 1, 0, 1, 3)
    ...
    # Handler methods for Group 14: Camera Profiles
